...
{
#   pragma omp parallel for default(none) shared(N, BD)
    for(int i=0; i<N; i++)
    {
        if (BD[i].m==0)
        {
...
#   pragma omp parallel for default(none) shared(collision_number, BD, ndel, collision_pair_1, collision_pair_2)
    for(int k=0; k<collision_number; k++)
    {
        int i = collision_pair_1[k];
        int j = collision_pair_2[k];
        if(i != j)
        {
