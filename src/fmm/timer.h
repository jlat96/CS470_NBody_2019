...
#ifdef _OPENMP
#   include <omp.h>
#   define INIT_TIMER(X) double _timer_ ## X = 0;
#   define START_TIMER(X) double _timer_ ## X ## _temp = omp_get_wtime();
...
#   include <sys/time.h>
    struct timeval _tv;
#   define INIT_TIMER(X) double _timer_ ## X = 0;
#   define START_TIMER(X) gettimeofday(&_tv, NULL); double _timer_ ## X ## _temp = _tv.tv_sec+(_tv.tv_usec/1000000.0);
