...
const static int DEFAULT_TIME_STEPS        = 1;
...
void init_defaults(t_fmm_params* params)
{
    ...
    params->timesteps      = DEFAULT_TIME_STEPS;
}
...
void print_help() 
{
    ...
    printf("    -s  --timesteps     100     Number of time steps\n");
    printf("Notes:\n");
...
void parse_fmm_args(int argc, char** argv, t_fmm_params* params)
{
    static struct option long_params[] = 
    {
        ...
        {"timesteps",   required_argument,  0,  's'}
    };
    ...
    while ((c = getopt_long(argc, argv, "hn:c:t:e:m:d:i:s:", long_params, NULL)) != -1)
    {
        switch(c)
        {
            case 'h':
    ...
    params->timesteps          = (tmp_params.timesteps     == DEFAULT_TIME_STEPS     ) ? params->timesteps      : tmp_params.timesteps;
